...
class CredStashController:
    ...
    def __init__(self, access_key_id, secret_access_key, default_region,
                 default_table, namespaces):
        ...
        for secret, keys in spec.items():
            ...
            try:
                ...
                secret_obj = client.V1Secret(api_version, {}, "Secret",
                                             metadata)
            if new or secret_obj.metadata.annotations.get(
                    "credstash-fully-managed", None) == "true":
                secret_obj.data = {}
    ...
    def process_event(self, event):
        ...
        if self.namespaces is not None and namespace \
                not in self.namespaces:
            print("Secret requested from an "
    ...
    def delete_secret(self, credstash_secret):
        ...
        for secret, keys in spec.items():
            ...
            if secret_obj.metadata.annotations.get(
                    "credstash-fully-managed",
                    None) == "true":
                self.v1core.delete_namespaced_secret(secret, namespace,
                                                     V1DeleteOptions())
...
if __name__ == "__main__":
    ...
    credstash_controller = CredStashController(main_access_key_id,
